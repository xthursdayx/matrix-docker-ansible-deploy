...
matrix_registration_admin_api_shared_secret: "ENTER_SOME_SECRET_HERE"
```
...
--extra-vars="max_usage=1 expiration_date=2021-12-31"
```

The above command creates and returns a **one-time use** token, which **expires** on the 31st of December 2021.
Adjust the `max_usage` and `expiration_date` variables as you see fit.
